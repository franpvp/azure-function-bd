...
public class TradeFunction {
    ...
    public HttpResponseMessage crearTrade(
        ...
        try {
            ...
            if (trade.getCanal() == null) {
                return badJson(request, "El canal es obligatorio");
            }
    ...
    private static Long insertarTrade(String url, String user, String pass, TradeDto trade, ExecutionContext ctx) throws Exception {
        try (Connection conn = DriverManager.getConnection(url, user, pass)) {
            ...
            String sql = "INSERT INTO TRADE (MONTO, CANAL, FECHA_CREACION, ID_CLIENTE) VALUES (?, ?, ?, ?)";
            try (PreparedStatement ps = conn.prepareStatement(sql, new String[]{"ID_TRADE"})) {
                ...
                ps.setString(2, trade.getCanal());
                ps.setDate(3, java.sql.Date.valueOf(trade.getFechaCreacion()));
                ps.setLong(4, trade.getIdCliente());
